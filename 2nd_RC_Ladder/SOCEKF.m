...
function [SOC_k, V1_k, V2_k] = SOCEKF(Cell_Data, dt, Case)
persistent F Q R P H K P_before
persistent x_esti x_esti_before Init_EKF

if isempty(Init_EKF)
    F = [1 0 0;                       % Linearized State Equation
         0 exp(-dt/(Cell_Data.R1 * Cell_Data.C1)) 0 
         0 0 exp(-dt/(Cell_Data.R2 * Cell_Data.C2))];

    if Case == "CHG"
        Q = [0.00001 0 0;          % System Noise. Estimation Value`s Variance Increases if Q Increased
              0 0.00001 0;
              0 0 0.00001];               
        ...
        P_before = [500 0 0;
                    0 500 0;
                    0 0 500];      % Estimation Error Covariance
    elseif Case == "DHG"
        Q = [0.001 0 0;            % System Noise
              0 0.001 0;
              0 0 0.001];               
